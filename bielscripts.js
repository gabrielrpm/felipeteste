    ...
    $("#linkteste").on('click tap', function (evt) {
      evt.preventDefault();
    ...
    setTimeout(function(e){ 
     window.location = $("#linkteste").attr("href");  
    }, 6000);   
    }); 

    $("#linkteste2").on('click tap', function (evt) {
      evt.preventDefault();
      $("body").prepend( '<div style="display:none" class="cupomcardpopup" id="demo"></div>' );
      $('html, body').css({
        overflow: 'hidden',  
    ...
    $("#kit1").on('click tap', function (evt) {
      evt.preventDefault();
      $("body").prepend( '<div style="height:100%; width:100%; background:#fff;position:fixed;z-index:20" id="de32mo"></div>' );
      $("#de32mo").hide();
      $("#de32mo").fadeTo(1000, 0.96);
     $("body").prepend( '<div id="circlecontainer"><div class="circle"><strong></strong></div></div>' );
    ...
    setTimeout(function(e){ 
     window.location = $("#kit1").attr("href");  
    }, 4500);   
    });

    $("#kit2").on('click tap', function (evt) {
      evt.preventDefault();
      $("body").prepend( '<div style="height:100%; width:100%; background:#fff;position:fixed;z-index:20" id="de32mo"></div>' );
      $("#de32mo").hide();
      $("#de32mo").fadeTo(1000, 0.96);
    ...
    setTimeout(function(e){ 
     window.location = $("#kit2").attr("href");  
    }, 4500);   
    });

    $("#kit3").on('click tap', function (evt) {
      evt.preventDefault();
      $("body").prepend( '<div style="height:100%; width:100%; background:#fff;position:fixed;z-index:20" id="de32mo"></div>' );
      $("#de32mo").hide();
      $("#de32mo").fadeTo(1000, 0.96);
    ...
    setTimeout(function(e){ 
     window.location = $("#kit3").attr("href");  
    }, 4500);   
    });

   /*  $(window).scroll(function() {
      var windowBottom = $(this).scrollTop() + $(this).innerHeight();
      $(".triangulodesconto").each(function() {
        var objectBottom = $(this).offset().top + $(this).outerHeight() + 40;
...
function carregaComment(){
  var x = document.getElementById("carregaComment"); 
  x.innerHTML = "Carregando...";
  setTimeout(function(){ 
    x.innerHTML = "Falha ao carregar..."; 
    x.style.backgroundColor = "#2f2f2f";
    x.style.color = "#fff";
}, 5000); 
